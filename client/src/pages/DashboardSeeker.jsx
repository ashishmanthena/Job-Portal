    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 py-12">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 mt-2">Track and manage all your job applications</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="card p-6">
            ...
            <p className="text-3xl font-bold text-blue-400">{apps.filter(a => a.status === 'viewed').length}</p>
          </div>
          <div className="card p-6">
                ...
                <div key={a._id} className="p-6 hover:bg-slate-700/30 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-white">{a.job?.title}</h3>
                      <p className="text-gray-400">{a.job?.company}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-blue-500/20">
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-300">{a.job?.location}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 mb-1">Applied</p>
                      <p className="text-sm font-medium text-gray-300">{new Date(a.createdAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 mb-1">Last Update</p>
                      <p className="text-sm font-medium text-gray-300">{new Date(a.updatedAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                    </div>
                  </div>
                </div>
              ))}
